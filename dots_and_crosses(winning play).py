...
Algorithm:
    ...
    class Winning_Player:
        __init__:
            initialize a box coordinate list
        __str__
        play_move:
            generate a specific winning move using a set of rules
        score:
...
class WinningPlayer(object):
    '''
    Create a player to play winning moves with a set of rules
    '''
    
    def __init__(self, size, code):
        '''
        initialize player score, board size and player code
        initialize box list consisting of box coordinates
        '''
        ...
        #create a list of box coordinates

        self.box_list = []

        for ver in range(self.size-1):
            for hor in range(self.size -1):
                l1 = [row_list[ver][hor], row_list[ver][hor+1]]
                l2 = [row_list[ver][hor], row_list[ver+1][hor]]
                l3 = [row_list[ver][hor+1], row_list[ver+1][hor+1]]
                l4 = [row_list[ver+1][hor], row_list[ver+1][hor+1]]
                
                self.box_list.append([l1, l2, l3, l4])
    ...
    def play_move(self, move_list, brd):
        '''
        return moves under a certain set of rules to make the best possible
        moves
        '''
        
        # if 3 lines of a box are completed, return the 4th line
        for i in self.box_list:
            if len(i) == 1:
                ret = i[0]
                self.box_list.pop(self.box_list.index(i))
                brd.update_board(ret) # update board
                return ret
            
        # if 1 line of box is completed, return the 2nd line
        for i in self.box_list:
            if len(i) == 3:
                ret = i[0]
                for j in self.box_list:
                    for k in j:
                        if ret == k:
                            idx = j.index(k)
                            j.pop(idx)
                brd.update_board(ret)
                return ret
            
        # if no lines of a box are completed, return the 1st line
        for i in self.box_list:
            if len(i) == 4:
                ret = i[0]
                for j in self.box_list:
        ...
        while True:
            ...
            if brd.is_valid_move(new_move, move_list):
...
class Game(object): 
    ...
    def single_play(self, fp):
        ...
        p2 = WinningPlayer(self.size, 'B')   #Initialize player 2
